...
export function configUpdate(
  ...
  Result<
    components.SaveUserConfigResponse,
    | errors.ErrorT
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.PutConfigByIdRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.SaveUserConfigRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/config/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.SaveUserConfigResponse$inboundSchema),
